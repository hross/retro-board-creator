...
export async function tryCreateRetro(args: IRetroArguments) {
    ...
    const retroDate: Date = nextRetroDate(lastRetroOn, args.retroDayOfWeek, args.retroCadenceInWeeks);
...
function nextRetroDate(lastRetroDate: Date, retroDayOfWeek: number, retroCadenceInWeeks: number): Date {
  // approximate the date of the next retro based on frequency
  let nextRetroDate = new Date(lastRetroDate.getDate() + retroCadenceInWeeks * 7);
  // make sure it's on the right day, in case the day of week changed
  nextRetroDate.setDate(nextRetroDate.getDate() + (7 + retroDayOfWeek - nextRetroDate.getDay()) % 7);

  return nextRetroDate;
}

// create the retro board and return the URL
async function createBoard(client: github.GitHub, retroDate: Date): Promise<string> {
...
async function createTrackingIssue(client: github.GitHub, projectUrl: string, retroDriver: string): Promise<number> {
